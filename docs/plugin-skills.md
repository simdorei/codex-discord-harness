...
| --- | --- | --- |
| `ask-chatgpt-pro` | Reuse one normal ChatGPT Pro conversation per Codex thread, bind that conversation to the originating local project through MCP, and create a new conversation when the saved one cannot be found. | `!pro <question>` invokes the skill from a mapped Discord thread. |
